...
else if (text_printing_state == PRINTING_STATE.PRINTING)
{
...
switch (dialogue_current_type)
{
	case DIALOGUE_TYPE.ONLY_TEXT:
		...
		{
			// Draw next arrow
			draw_sprite_ext(spr_next_arrow, image_index, next_arrow_top_left.x, next_arrow_top_left.y, 1, 1, 0, c_white, 1);
		}
		break;
	...
	case DIALOGUE_TYPE.PORTRAIT_LEFT:
		...
		// Character Sprite
		draw_sprite_ext(current_potrait_sprite, img_index, inner_portrait_top_left.x, inner_portrait_top_left.y, (const_inner_portrait_width / sprite_get_width(current_potrait_sprite)) * opening_anim_scaler * _flip_float, (const_inner_portrait_height / sprite_get_height(current_potrait_sprite)) * opening_anim_scaler, 0, c_white, 1);
		// Check if the text has finished printing to show next arrow
		if (text_printing_state == PRINTING_STATE.FINISHED)
		{
			// Draw next arrow
...
// Draw Text
if (dialogue_current_state == DIALOGUE_STATE.ACTIVE || dialogue_current_state == DIALOGUE_STATE.PAUSED)
{
